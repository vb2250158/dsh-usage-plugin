...
- **Top conversations by cost**: the overview's bottom section lists the 10 costliest conversations (by cost, ties broken by the most recent call), each row showing rank, conversation name, a cost bar, call count and token total, and the amount. **The name comes from the host's session list** — the durable title when there is one, otherwise the project folder name, and the session id only as a last resort — so it matches what the sidebar shows. **Clicking a name jumps to that conversation**, and closes the settings panel when the click came from there. Conversations the host list no longer carries (older records) stay plain text and are not clickable.
