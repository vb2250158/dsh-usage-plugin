...
> **本仓库是本机维护的自有版本，不是上游包。**
> 开发位置 [vb2250158/dsh-usage-plugin](https://github.com/vb2250158/dsh-usage-plugin)，
> 独立于上游 [feiyang-dev/dsh-usage-plugin](https://github.com/feiyang-dev/dsh-usage-plugin)
> （后者保留为 `upstream` remote 仅作参考）。包名是不带 scope 的 `dsh-usage-plugin`，
> 按固定提交直接从本仓库安装，不发布到 npm：
>
> ```sh
...
> ### 关于本 fork
>
> 本仓库是 [`feiyang-dev/dsh-usage-plugin`](https://github.com/feiyang-dev/dsh-usage-plugin) 的
> **下游 fork**（MIT，原作者列在 `package.json`）。上游的用量采集引擎原样保留 —— 包括
> `llm/stream` 监听、缓存写入兜底与中断调用记账 —— 只把 DeepSeek 专属的价格表换成**通用目录**，
> 让任何模型都能计价。
>
> 本 fork 的改动（v1.17.0）：
>
> - **计价改为目录驱动。** `syncPrices` 拉取 `https://openrouter.ai/api/v1/models`，按结果给每次
>   调用计价。费用不再受 provider 白名单限制 —— 只要模型在表里，走哪条通道都能算。
> - **模型按名称匹配，不按写死的档位。** 记录的 `model` 依次尝试精确 id → 型号名 → 别名，
>   因此 `gpt-6-astra` 能命中 `openai/gpt-6-astra`。匹配不上的模型**刻意记 0** ——
>   宁可留下可见的缺口，也不要拿相似型号的价格冒充。
> - **货币跟随价格来源。** 价格是美元（OpenRouter 的计价单位），金额符号跟 `currency` 设置走。
>   写死的 `¥` 和 USD/CNY 换算步骤都已移除，它们原本只为服务 DeepSeek 的人民币价格表。
> - **缓存计价正确。** 缓存命中的输入价取 `input_cache_read`；当 OpenRouter 该字段缺失或为 `0`
>   （不支持缓存的模型就是这样）时退回未命中价 —— 把那个 `0` 当真会把缓存输入算成免费，
>   整体偏低。
> - **计价逻辑抽到 `lib/pricing.js`**，让规则可以被单元测试，而不是困在插件的 `apply()` 闭包里。
>   `npm test` 跑 42 个用例。
>
> **仍沿用 DeepSeek 模型的部分**（无害，但已无意义）：高峰 / 空闲的**花费拆分**。两档现在存的是
> 同一个目录平价，所以「高峰 / 空闲」变成了同一笔总额的时间分档。余额查询页签与
> `EFFECTIVE_AT` 价格生效时间开关同属 DeepSeek 遗留。
>
> 上游归原作者所有，上游问题请提给上游；与 OpenRouter 计价层相关的问题提到本仓库。

---

> ## 🔔 重要通知（2026-08-16）：npm 包名已更换
>
> 本插件的 **npm 包名已由 `@feiyang666/deepseekharnessdesktop` 更名为 `dsh-usage-plugin`**（与 GitHub 仓库名 `feiyang-dev/dsh-usage-plugin` 保持一致）。
>
> - 安装 / 升级请使用新包名：`dsh plugin --profile web add dsh-usage-plugin`
...
- **会话消耗排行**：概览底部列出消耗最高的 10 个会话（按消耗降序，同额时按最近调用），每行给出序号、会话名、消耗长条、调用次数与 token 量、金额。**会话名取自宿主会话列表**（有标题用标题，没有则退回项目目录名，最后才退回会话 id），与侧栏里看到的一致；**点击会话名可直接跳转到该会话**，从设置面板点进去时会一并收起设置。不在宿主列表里的会话（记录里更早、列表已不包含的那些）保持纯文本、不可点击。
...
```bash
cd ~/.dsh/profiles/web
pnpm update dsh-usage-plugin   # 或：npm update dsh-usage-plugin
```
...
> ⚠️ **不要手动修改 `~/.dsh/profiles/web/node_modules/dsh-usage-plugin/` 下的文件**（如 `lib/index.js` / `lib/client.js`）。每次更新都会从 npm 重新解包并覆盖这些文件，本地改动会被静默丢弃。如需改行为，请 fork 仓库自行发布版本或向上游提贡献。
