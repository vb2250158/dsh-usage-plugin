...
- **会话消耗排行的每一行改用会话名**：此前直接显示 `sessionId`（如 `session-22d1c6d…`），
  只能靠前缀去猜是哪一个。现在从宿主会话列表取 `displayTitle` —— 有标题用标题，没有则
  退回项目目录名，最后才退回会话 id —— 与侧栏里显示的名字一致。截断长度仍是 26 字符，
  完整名字与原始 id 都放在悬停提示里，方便核对名字和查日志。

- **点击会话名可直接跳转到该会话**：调用客户端会话服务的 `open(id)`，并把设置面板一并
  收起 —— 否则跳转结果被面板挡着，看不见。鼠标移上去才浮出链接外观（与 token 胶囊同一
  种克制的做法），平时与普通文本无异。

- **不在宿主列表里的会话保持纯文本、不可点**：用量记录里有相当一部分会话在宿主列表里
  找不到（实测当前 56 个里有 16 个，是带不带 `session-` 前缀两种 id 形态的差别），而
  `open()` 对未知 id 会直接抛错。这些行照旧显示 id —— 留一个点了没反应的假链接比不可点更糟。

### 备注

- 会话名与跳转动词分别取自宿主全局标准 prop `useSessions` 与 `sessions` 服务，
  不新增任何跨包依赖；`settings.section` 与 `conversation.view` 两个挂载点共用同一实现。
- 「显示哪个名字、能不能点」抽成了纯函数 `sessionRowView`，由 `test/session-row.test.js`
  覆盖（6 个用例）。测试总数 42 → **48 全通过**。
- 顺带删掉了一处重复的 `exports.__i18n` 赋值（同一对象被赋了两次，第二次是冗余）。

---

## v1.18.0 (2026-09-15) — token 显示口径统一为官方格式

### 界面 / UI

- **所有 token 数字改用官方紧凑单位**：全插件 54 处 token 显示从千分位原值
  （`2,355,681`）改为官方 `ui-chat` 的紧凑写法（`2.4M`），并统一带 ` tok` 后缀，
  与官方输入框下方的胶囊 `46.5M tok · 缓存命中 97%` 完全同款。
  转换算法逐字复刻 `packages/client/ui-chat/src/client/chat/token-format.ts`
  的 `formatTokens()`：`517 / 12.2K / 517K / 1.2M`，即千位以下原样、
  `K`/`M` 分段、刻度值小于 100 保留一位小数、达到 100 则取整。
  已按官方 locale 逐值对拍（`scripts/verify-token-format.mjs`，18 个用例全通过）。

- **消息胶囊改写为官方格式**：助手消息操作栏上的胶囊从 `本轮 token 2,355,681`
  改为 `2.4M tok · 缓存命中 98%`——数字用紧凑单位，缓存命中率以整数百分比跟在
  后面，与官方胶囊同构。悬停 `title` 仍给出精确值（`2,355,681 tok · 98.2%`），
  信息不因压缩而丢失。

- **胶囊平时呈静态文本，悬停/聚焦才浮出按钮外观**：未指上去时没有边框和底色，
  指针移入（或键盘 `focus-visible`）才浮现按钮的边框与浅底色，`active` 时再加深
  一档。避免这段文字在不需要交互时也占一圈边框。

- **缓存命中率文案对齐官方**：概览大卡、弹窗进度条、弹窗「本轮明细」与「对话累计」
  四处从 `缓存命中率 98.2%` 改为官方的 `缓存命中 98%`（整数、去掉「率」字），
  与 `ui-chat` 的 `'stats.cacheHit': '缓存命中 {percent}%'` 一致。

### 未改动的部分 / Unchanged

- **PNG 导出报告仍用千分位原值**（`ctx.fillText` 路径）。该报告用于与官方后台对账，
  紧凑单位会丢失对账所需的精确数字，故这一条出口有意保留。
- **调用次数、记录条数、模型个数等计数仍用 `fmtInt`**（`1,234 次` 而非 `1.2K 次`），
  这些是次数而非 token 数量。
