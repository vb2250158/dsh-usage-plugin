window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var st = {
      ...
      // 会话名可跳转：平时与普通文本无异，悬停才浮出链接外观（规则在文件头注入的样式里），
      // 免得整列都在喊「点我」。不可跳转的行不带这个 cursor。
      sessionNameLink: { cursor: "pointer" },
    ...
    // 紧凑 token 计数，逐字复刻 ui-chat 的 token-format.ts#formatTokens：
    // 517 / 12.2K / 517K / 1.2M（十位以下保留一位小数，百位以上取整）。
    // 与官方胶囊 `6.6M tok · 缓存命中 97%` 的显示口径保持一致。
    ...
    // ── token 数字的唯一显示口径 ─────────────────────────────────────────
    // 全插件的 token 数字一律走 fmtTok（紧凑单位 + ` tok` 后缀），与官方
    // StatsPills 的 `46.5M tok · 缓存命中 97%` 保持同一口径。精确千分位值
    // 只在两处保留：胶囊与弹窗明细的 title 悬停提示，以及 PNG 对账报告。
    ...
    // 价格表以 OpenRouter 的美元价为准，因此金额符号跟随后端下发的 currency。
    // 不能硬编码 ¥：那会把美元金额按人民币符号显示，数值看着合理但币种是错的。
    var displayCurrency = "USD";
    var setDisplayCurrency = function (c) { displayCurrency = c === "CNY" ? "CNY" : "USD"; };
    var fmtMoney = function (n) {
      var s = displayCurrency === "CNY" ? "¥" : "$";
      if (!n) return s + "0.0000";
      if (n < 0.0001) return s + n.toExponential(2);
    ...
    // 价格表的键现在是任意模型 id，标签就是 id 本身；只有真正取不到模型名时
    // 才退回「未知模型」。
    var modelLabel = function (mk) { return mk && mk !== "unknown" ? String(mk) : t("未知模型"); };
...
var api = function (payload) {
    ...
    // 价格表的模型 id 由后端下发（props.priceModels，来自 OpenRouter 同步）。
    // 这里保留空默认值，避免早期代码路径读到 undefined。
    var PRICE_MODELS = [];
    ...
    function drawReport(canvas, records) {
      ...
      var cards = [
        { l: t("调用次数"), v: fmtInt(count) }, { l: t("输入 · 未命中"), v: fmtTok(totalMiss) + " tok" },
        { l: t("输入 · 缓存命中"), v: fmtTok(totalHit) + " tok" }, { l: t("输出"), v: fmtTok(totalOut) + " tok" },
      ...
      for (var r3 = 0; r3 < count; r3++) {
        ...
        ctx.textAlign = "right";
        ctx.fillText(fmtTok(miss) + " tok", c1.missR, ty);
        ctx.fillStyle = "#22a45d"; ctx.fillText(fmtTok(hit) + " tok", c1.hitR, ty);
    ...
    // ── 会话消耗排行 Top 10 ──
    // useSessions 缺席时的占位（只可能出现在单测的假 props 上）：恒返回 null。
    // 用它兜底是为了让顶层那个 hook 的调用次数在两次渲染之间保持一致。
    function noSessionsFeed() { return null; }

    var SESSION_LABEL_MAX = 26;

    // 一行会话的展示决定：叫什么都、能不能点。抽成纯函数是因为本仓的测试只加载
    // factory（假 React 的 createElement 返回 null），组件本身渲染不出来 ——
    // 「用会话名还是退回首 id」这条判断必须在组件之外可测。
    // @param sid 用量记录里的原始会话 id
    // @param info host 列表 byId 里的那一行，可能不存在
    // @param canOpen 使用方是否提供了跳转动词
    // @returns label 是截断后的显示文本，hint 是 title 全文，linkable 决定是否挂点击
    function sessionRowView(sid, info, canOpen) {
      var raw = String(sid || "");
      var name = info && info.displayTitle ? String(info.displayTitle) : "";
      // 只有 host 列表里真有这一行才给点：sessions.open() 对未知 id 会直接抛错，
      // 留个点了没反应的假链接比不可点更糟。
      var linkable = !!(name && canOpen);
      var label = name || raw;
      var short = label.length > SESSION_LABEL_MAX ? label.slice(0, SESSION_LABEL_MAX) + "…" : label;
      // title 里带上完整会话名与原始 id：名词被截断后仍能核对是哪一个，id 便于对日志。
      var hint = name ? name + "\n" + raw : raw;
      if (linkable) hint += "\n" + t("点击跳转到该会话");
      return { label: short, hint: hint, linkable: linkable };
    }

    function SessionRank(props) {
      var records = props.records || [], regime = props.regime || "auto";
      // 会话身份由使用方注入：sessionMeta 是 host 列表的 byId（含 displayTitle），
      // openSession 是跳转动词。两者缺一 —— 或某个会话不在 host 列表里 —— 就退回
      // 原来的「显示 sessionId、不可点」，而不是留一个点了没反应的假链接。
      var sessionMeta = props.sessionMeta || null;
      var openSession = typeof props.openSession === "function" ? props.openSession : null;
      var bySid = {};
      ...
      return el("div", { style: st.sessionCard },
        ...
        rows.map(function (row, idx) {
          ...
          var view = sessionRowView(row.sid, sessionMeta ? sessionMeta[row.sid] : null, !!openSession);
          return el("div", { key: row.sid, style: st.sessionRow },
            el("span", {
              style: view.linkable ? Object.assign({}, st.sessionName, st.sessionNameLink) : st.sessionName,
              title: view.hint,
              "data-dsh-usage-sessionlink": view.linkable ? "" : undefined,
              onClick: view.linkable ? function () { openSession(row.sid); } : undefined
            }, (idx + 1) + ". " + view.label),
            el("div", { style: st.sessionBar },
              el("div", { style: Object.assign({}, st.sessionBarFill, { width: pctv + "%" }) })
            ),
            el("span", { style: st.sessionSub, title: t("总 token") + ": " + fmtTok(row.tokens) + " tok" }, fmtInt(row.calls) + t(" 次 · ") + fmtTok(row.tokens) + " tok" + " tok"),
    ...
    function OverviewView(props) {
      ...
      var tokenTotals = [
        ...
        { l: t("输入 · 未命中"), v: fmtTok(totalMiss) + " tok" },
        { l: t("缓存命中"), v: fmtTok(totalHit) + " tok" },
        { l: t("输出"), v: fmtTok(totalOut) + " tok" },
        { l: t("推理"), v: totalReason ? fmtTok(totalReason) + " tok" : "—" }
      ];
      var modelRows = [];
      for (var k in byModel) modelRows.push(byModel[k]);
      modelRows.sort(function (a, b) { return a.key < b.key ? -1 : a.key > b.key ? 1 : 0; });
      ...
      for (var gi = 0; gi < providerGroups.length; gi++) {
        ...
        pmRows.push(el("tr", { key: "g-" + gprov2.key },
          ...
          el("td", { style: st.tdGroupR }, fmtTok(gprov2.miss) + " tok"),
          el("td", { style: st.tdGroupR }, fmtTok(gprov2.hit) + " tok"),
          el("td", { style: st.tdGroupR }, fmtTok(gprov2.out) + " tok"),
          el("td", { style: st.tdGroupR }, gprov2.reason ? fmtTok(gprov2.reason) + " tok" : "—"),
        ...
        for (var mi = 0; mi < g.models.length; mi++) {
          ...
          pmRows.push(el("tr", { key: "pm-" + md.key },
            ...
            el("td", { style: st.td }, fmtTok(md.miss) + " tok"),
            el("td", { style: st.td }, el("span", { style: st.badgeHit }, fmtTok(md.hit) + " tok")),
            el("td", { style: st.td }, fmtTok(md.out) + " tok"),
            el("td", { style: st.td }, md.reason ? fmtTok(md.reason) + " tok" : "—"),
      ...
      pmRows.push(el("tr", { key: "g-total" },
        ...
        el("td", { style: st.tdTotal }, fmtTok(totalMiss) + " tok"),
        el("td", { style: st.tdTotal }, fmtTok(totalHit) + " tok"),
        el("td", { style: st.tdTotal }, fmtTok(totalOut) + " tok"),
        el("td", { style: st.tdTotal }, totalReason ? fmtTok(totalReason) + " tok" : "—"),
      ...
      return el("div", null,
        ...
        el("div", { style: st.bigCards },
          ...
          el("div", { style: st.bigCard },
            el("div", { style: st.bigCardL }, t("缓存命中")),
            el("div", { style: st.bigCardV }, hitRate),
            el("div", { style: st.bigCardH }, t("输入 · 缓存命中 ") + fmtTok(totalHit) + " tok")
          )
        ),
        el("div", { style: st.cards },
          el(Card, { label: t("输入 · 未命中"), value: fmtTok(totalMiss), hint: "token" }),
          el(Card, { label: t("输出"), value: fmtTok(totalOut), hint: "token" }),
              ...
              el("tbody", null,
                modelRows.map(function (m) {
                  return el("tr", { key: m.key },
                    ...
                    el("td", { style: st.td }, fmtTok(m.miss) + " tok"),
                    el("td", { style: st.td }, el("span", { style: st.badgeHit }, fmtTok(m.hit) + " tok")),
                    el("td", { style: st.td }, fmtTok(m.out) + " tok"),
                    el("td", { style: st.td }, m.reason ? fmtTok(m.reason) + " tok" : "—"),
    ...
    function CalendarView(props) {
      ...
      return el("div", null,
        ...
        el("div", { style: st.cards, marginTop: 12 },
          ...
          el(Card, { label: t("输入 · 未命中"), value: fmtTok(monthMiss), hint: "token" }),
          el(Card, { label: t("输入 · 缓存命中"), value: fmtTok(monthHit), hint: "token" }),
          el(Card, { label: t("输出"), value: fmtTok(monthOut), hint: "token" }),
              ...
              el("table", { className: "dsh-usage-table collapse-mobile", style: st.tbl },
                ...
                el("tbody", null, monthRows.map(function (drow) {
                  ...
                  return el("tr", { key: drow.day },
                    ...
                    el("td", { style: st.td }, fmtTok(drow.miss) + " tok"),
                    el("td", { style: st.td }, el("span", { style: st.badgeHit }, fmtTok(drow.hit) + " tok")),
                    el("td", { style: st.td }, fmtTok(drow.out) + " tok"),
                    ...
                    el("table", { className: "dsh-usage-table collapse-mobile", style: st.tbl },
                      ...
                      el("tbody", null, selRecords.map(function (rr) {
                        ...
                        return el("tr", { key: rr.time },
                          ...
                          el("td", { style: st.td }, fmtTok(miss2) + " tok"),
                          el("td", { style: st.td }, el("span", { style: st.badgeHit }, fmtTok(hit2) + " tok")),
    ...
    function CacheListView(props) {
      ...
      return el("div", null,
        ...
        el("div", { style: st.note, marginTop: 8 },
          t("当前范围：共 ") + fmtInt(filtered.length) + t(" 条 · 输入·未命中 ") + fmtTok(sumMiss) + " tok" + t(" · 缓存命中 ") + fmtTok(sumHit) + t(" · 缓存写入 ") + fmtTok(sumWrite) + t(" · 输出 ") + fmtTok(sumOut) +
                  ...
                  el("tbody", null,
                    pageRows.map(function (rr3, idx) {
                      ...
                      return el("tr", { key: rr3.time + "-" + idx },
                        ...
                        el("td", { style: st.td }, fmtTok(miss3) + " tok"),
                        el("td", { style: st.td }, el("span", { style: st.badgeHit }, fmtTok(hit3) + " tok")),
                    ...
                    el("tr", null,
                      ...
                      el("td", { style: st.tdTotal }, fmtTok(sumMiss) + " tok"),
                      el("td", { style: st.tdTotal }, fmtTok(sumHit) + " tok"),
                      el("td", { style: st.tdTotal }, fmtTok(sumWrite) + " tok"),
                      el("td", { style: st.tdTotal }, fmtTok(sumOut) + " tok"),
                      el("td", { style: st.tdTotal }, sumReason ? fmtTok(sumReason) + " tok" : "—"),
    ...
    function PriceView(props) {
      ...
      // 价格表的键是任意模型 id（由 OpenRouter 同步填充），所以模型列表必须来自
      // 数据本身；写死档位名会让同步来的 400 多个模型一个都不显示。
      var priceModels = (props.priceModels && props.priceModels.length)
        ? props.priceModels
        : Object.keys((pricing && pricing.base) || {});
      var table = (pricing && pricing.base) || {};
      var meta = props.priceMeta || {};
      var filterState = React.useState("");
      var filter = filterState[0], setFilter = filterState[1];
      var syncState = React.useState({ busy: false, msg: "" });
      var sync = syncState[0], setSync = syncState[1];
      var MAX_ROWS = 200;
      var q = String(filter || "").trim().toLowerCase();
      var ids = q ? priceModels.filter(function (id) { return String(id).toLowerCase().indexOf(q) >= 0; }) : priceModels;
      var rows = ids.slice(0, MAX_ROWS).map(function (id) {
        var row = table[id] || {};
        return el("tr", { key: id },
          el("td", { style: st.tdFirst }, String(id)),
          el("td", { style: st.td }, el("span", { style: st.badgeHit }, fmtPrice(row.cacheHit))),
          el("td", { style: st.td }, fmtPrice(row.cacheMiss)),
          el("td", { style: st.td }, fmtPrice(row.output))
        );
      });
      function doSyncPrices() {
        setSync({ busy: true, msg: "" });
        var p = props.onSyncPrices ? props.onSyncPrices() : Promise.resolve({ ok: false, error: "no handler" });
        p.then(function (r) {
          setSync({ busy: false, msg: r && r.ok
            ? t("已同步 ") + (r.count || 0) + t(" 个模型的价格")
            : t("同步失败：") + ((r && r.error) || t("未知错误")) });
        }).catch(function (e) { setSync({ busy: false, msg: t("同步失败：") + String((e && e.message) || e) }); });
      }
      ...
      return el("div", null,
        ...
        el("div", { style: st.sec, marginTop: 18 }, t("模型价格表")),
        el("div", { style: st.note, marginTop: 6 },
          t("价格来自 OpenRouter 公开目录，单位：") + (props.currency || "USD") + t(" / 百万 tokens。同步后即可为每条调用自动计价；表中查不到的模型费用按 0 计，不做近似匹配。")),
        el("div", { style: st.actions, marginTop: 8 },
          el("button", { style: st.btnPrimary, disabled: sync.busy, onClick: doSyncPrices }, sync.busy ? t("同步中…") : t("同步价格")),
          el("input", { style: st.numInput, placeholder: t("搜索模型…"), value: filter, onChange: function (e) { setFilter(e.target.value); } }),
          el("span", { style: st.note }, t("共 ") + priceModels.length + t(" 个模型")),
          sync.msg ? el("span", { style: st.note }, sync.msg) : null
        ),
        el("div", { style: st.note, marginTop: 4 },
          meta.syncedAt
            ? t("上次同步：") + fmtTime(meta.syncedAt) + (meta.error ? t("；上次同步失败：") + meta.error : "")
            : t("尚未同步价格，点击「同步价格」从 OpenRouter 拉取。")),
        priceModels.length
          ...
          : null,
        ids.length > MAX_ROWS
          ? el("div", { style: st.note, marginTop: 6 }, t("仅显示前 ") + MAX_ROWS + t(" 个模型，用搜索框可缩小范围。"))
          : null,
        el("div", { style: st.note, marginTop: 8 },
          t("缓存命中价取 OpenRouter 的 input_cache_read；该字段缺失或为 0 时按未命中价处理，以免低估花费。"))
      );
    }
    // ── Usage panel ──
    var SUBTABS = [
    ...
    function UsagePanel(props) {
      ...
      var state = React.useState({ records: [], count: 0, dataPath: "", persistOk: false, persistError: "", pricing: null, priceModels: [], priceMeta: null, currency: "USD", days: [], budget: null });
      ...
      // 会话名与跳转：useSessions 是「delivered to EVERY slot component」的全局标准 prop，
      // 所以 settings.section 与 conversation.view 两个挂载点共用这一份实现。
      var sessionsById = (props.useSessions || noSessionsFeed)(function (s) { return s.byId; }) || null;
      ...
      function refresh() {
        api({ action: "list" }).then(function (res) {
          // 金额符号由后端下发的 currency 决定，必须在渲染前更新模块级变量。
          if (res && res.currency) setDisplayCurrency(res.currency);
          setData(res || { records: [], count: 0, dataPath: "", persistOk: false, persistError: "", pricing: null, priceModels: [], priceMeta: null, currency: "USD", days: [], budget: null });
          setError("");
          setLoading(false);
      ...
      function syncPrices() {
        return api({ action: "syncPrices" }).then(function (res) {
          refresh();
          return res;
        });
      }
      ...
      return el("div", { style: st.root },
        ...
        loading
          ? el(LoadingView, null)
          : el("div", null,
            tab === "overview" ? el(OverviewView, { records: records, regime: "auto", toolCalls: data.toolCalls || [], days: data.days || [], budget: data.budget, sessionMeta: sessionsById, openSession: props.openSession }) : null,
            ...
            tab === "prices" ? el(PriceView, { pricing: data.pricing, priceModels: data.priceModels || [], priceMeta: data.priceMeta || null, currency: data.currency || "USD", budget: data.budget, onBudgetChange: saveBudget, onSyncPrices: syncPrices, onChanged: refresh }) : null
          ),
        el("div", { style: st.actions, marginTop: 8 },
    ...
    // ── 本轮时间窗（纯函数，可被 node:test 覆盖）──────────────────────────
    // 在 ChatSnapshot 里定位承载该消息的节点，读出它的时间窗字段。字段名与
    // 官方 TurnTailChatData 对齐：closing.finalNode.time 是消息完成时间，
    // .timing.stepStartTime 是该步开始时间，timeline.turns[turn].start.time 是
    // 整轮开始时间。命中不到就返回 0，由调用方决定怎么办（绝不回退成"整场累计"）。
    function turnWindowField(snap, messageId, field) {
      if (!snap || !snap.nodes || typeof snap.nodes.values !== "function") return 0;
      var arr = snap.nodes.values();
      for (var ni = 0; ni < arr.length; ni++) {
        ...
        if (field === "time") return fin.time || 0;
        if (field === "step") return (fin.timing && fin.timing.stepStartTime) || 0;
        var turn = vh.data.turn;
        if (turn === undefined && vh.location) turn = vh.location.turn;
        var turns = snap.timeline && snap.timeline.turns;
        if (turn === undefined || !turns || typeof turns.get !== "function") return 0;
        var loc = turns.get(turn);
        return (loc && loc.start && loc.start.time) || 0;
      }
      return 0;
    }

    // 合成宿主查询窗口：整轮优先，退一步用所在步骤起点，再退一步用消息完成前
    // 5 分钟。三者全无（快照里没有这一轮）时 ok=false —— 调用方据此不发请求，
    // 而不是拿一个 0/0 的窗口去换回整场对话累计。
    function turnWindowFrom(msgTime, stepStart, turnStart) {
      var to = msgTime > 0 ? msgTime : 0;
      var from = turnStart > 0 ? turnStart
        : stepStart > 0 ? stepStart
          : msgTime > 0 ? msgTime - 5 * 60 * 1000 : 0;
      return { from: from, to: to, ok: from > 0 && to > 0 && from <= to };
    }

    // ── 弹窗内容渲染 ──────────────────────────────────────────────
    //
    // 这段原本是 MessageTokenAction 里的闭包族（stat / cacheBar / models /
    // turnStats / summary / body），闭包变量全靠词法作用域兜着，于是写错一个名字
    // ——比如 turnStats 里引用了只存在于 turnWindowFrom 形参表里的 msgTime——
    // 编译期和 lint 都看不见，要等真机上「有数据 + 展开弹窗」才炸，表现为整个
    // 弹窗空白。提成模块级纯函数后，依赖全部显式列在 view 上，测试可以直接喂
    // 一个 view 把它渲染一遍（见 test/client-render-smoke.test.js）。
    function renderTokenPanel(view) {
      var status = view.status;
      var convo = view.convo, agg2 = view.agg2, recs = view.recs, modelRows = view.modelRows;
      var convoTokens = view.convoTokens, turnTokens = view.turnTokens;

      // 数字统一走 fmtTok 压缩单位（与官方口径一致），精确值挂在 title 上，
      // 鼠标悬停仍能看到千分位原值，信息不丢失。
      function stat(label2, value2, exact2) {
        var valueProps = { style: __tok.statValue };
        if (exact2) valueProps.title = exact2;
        return el("div", { style: __tok.stat },
          el("div", { style: __tok.statLabel }, label2),
          el("div", valueProps, value2)
        );
      }
      ...
      function cacheBar() {
        ...
        return el("div", { style: __tok.barWrap },
          ...
          el("span", { style: __tok.barLabel }, t("缓存命中") + " " + (pct ? Math.round(pct) : "0") + "%")
        );
      }

      function models() {
        if (!recs.length) return null;
        var sorted = modelRows.slice().sort(function (x, y) { return (y.totalCost - x.totalCost) || (y.calls - x.calls); });
        var MAX_MODELS = 6;
        var shown = sorted.slice(0, MAX_MODELS);
        var extra = sorted.length - shown.length;
        var cards = shown.map(function (m, idx) {
          return el("div", { key: String(idx), style: __tok.mCard },
            ...
            el("div", { style: __tok.mLine },
              t("输入·未命中") + " " + fmtTok(m.input) + " tok" + " · " + t("缓存命中") + " " + fmtTok(m.cacheRead) + " tok" + " · " + t("输出") + " " + fmtTok(m.output) + " tok" + (m.reasoning ? " · " + t("推理") + " " + fmtTok(m.reasoning) + " tok" : "")
            ),
      ...
      function turnStats() {
        ...
        // 本轮窗口的终点减该步起点。stepStart / winTo 由调用方在 view 上显式传入。
        var stepStart = view.stepStart || 0;
        var span = (view.hasWindow && view.winTo > 0 && stepStart > 0 && view.winTo > stepStart)
          ? (view.winTo - stepStart)
          : ((agg2.time && agg2.time.end) ? Math.max(0, agg2.time.end - agg2.time.start) : 0);
        return el("div", { style: __tok.statRow },
          stat(t("本次输出"), fmtTok(agg2.output) + " tok", fmtInt(agg2.output)),
          stat(t("本轮 token"), fmtTok(turnTokens) + " tok", fmtInt(turnTokens)),
          stat(t("本轮消耗"), fmtMoney(agg2.totalCost)),
          stat(t("本轮耗时"), fmtDur(span)),
          stat(t("缓存命中"), (agg2.hitRate ? Math.round(agg2.hitRate) : "0") + "%")
        );
      }
      ...
      function summary() {
        ...
        return el("div", { style: __tok.statRow },
          stat(t("总 token"), fmtTok(convoTokens) + " tok", fmtInt(convoTokens)),
          stat(t("总消耗"), fmtMoney(convo.totalCost)),
          stat(t("耗时"), fmtDur(span)),
          stat(t("缓存命中"), (convo.hitRate ? Math.round(convo.hitRate) : "0") + "%")
        );
      }

      if (status === "loading") return el("div", { style: __tok.hint }, t("获取中…"));
      if (status === "error") return el("div", { style: __tok.hint }, t("查询失败"));
      if (status === "nowindow") return el("div", { style: __tok.hint }, t("本轮时间窗不可用（该轮不在已加载的会话快照中），不显示累计值"));
      ...
      // 顺序即语义：胶囊挂在消息尾巴上，展开第一眼必须是「本轮」——它才是
      // 用户点开的原因。会话累计退到最后，并显式标出它是「本会话」级别，
      // 免得又出现「同一个词指向两个量」的困惑。
      return el("div", null,
        ...
        convo ? el("div", null,
          el("div", { style: __tok.title2 }, t("本会话累计")),
          summary()
        ) : null,
    ...
    function MessageTokenAction(props) {
      ...
      // 本轮时间窗的三个读数全部来自 ChatSnapshot（ui-chat 的标准 hook
      // `useChat`）：nodes 是带 values() 的 keyed store，timeline.turns 给出每个
      // Turn 的起止。这里曾读 `snap.chat.nodes`，而 SessionSnapshot 上并没有
      // `chat` 字段（chat 由 ui-conversation 的 view 层装配），于是窗口恒为 0/0
      // —— 宿主不做时间过滤，「本轮」实际等于整场对话累计，胶囊把会话总量当本轮
      // 显示。读数只返回数字：hook selector 的返回值必须是原始值，否则
      // useSyncExternalStore 会因为引用每次不同而反复重渲染。
      var readWindow = function (field) {
        if (!props.useChat || typeof props.useChat !== "function") return 0;
        try {
          return props.useChat(function (snap) {
            return turnWindowField(snap, messageId, field);
          }) || 0;
        } catch (e) { return 0; }
      };
      var win = turnWindowFrom(
        readWindow("time"),
        readWindow("step"),
        readWindow("turnStart"),
      );
      var winFrom = win.from;
      var winTo = win.to;
      var hasWindow = win.ok;

      React.useEffect(function () {
        if (!sessionId) { setData({ status: "idle", data: null }); return; }
        if (!hasWindow || winFrom > winTo) { setData({ status: "nowindow", data: null }); return; }
        setData({ status: "loading", data: s1.data });
        api({ action: "tokenForMessage", sessionId: sessionId, from: winFrom, to: winTo }).then(function (res) {
          setData({ status: res && res.ok ? "done" : "error", data: res });
        }).catch(function () { setData({ status: "error", data: null }); });
      }, [messageId, sessionId, hasWindow, winFrom, winTo]);
      ...
      // 显示总量取后端 computeAgg 算好的 displayTotal（计费输入 + 输出），
      // 不在前端重算：口径只能有一处定义，否则迟早漂移。这里曾自行相加
      // input+cacheRead+cacheWrite+output+reasoning，而 reasoning 已被数据源
      // 含在 output 内，等于重复计数一次；同时漏了让缓存写入进入分母，
      // 命中率因此被高估。
      var convoTokens = convo ? (convo.displayTotal || 0) : 0;
      var turnTokens = agg2 ? (agg2.displayTotal || 0) : 0;
      // 胶囊文案对齐官方 StatsPills：`6.6M tok · 缓存命中 97%`。
      // 数字走 fmtTok 压缩单位，缓存命中率跟在后面；精确值留给 title 属性。
      var label = t("获取中…");
      var exactLabel = "";
      if (s1.status === "error") label = "—";
      if (s1.status === "nowindow") {
        // 快照没给出这一轮的时间窗。旧版这里会退回 conversation（整场累计），
        // 于是胶囊把会话总量当成「本轮」显示 —— 宁可空着，也不显示错的数。
        label = "—";
        exactLabel = t("本轮时间窗不可用");
      }
      if (s1.status === "done") {
        // 时间窗已限定在本轮，aggregate 即本轮；不再回退到 conversation 累计。
        var lens = agg2 ? { tok: turnTokens, rate: agg2.hitRate } : null;
        if (lens) {
          var hitTxt = (typeof lens.rate === "number" && lens.rate > 0)
            ? " · " + t("缓存命中") + " " + Math.round(lens.rate) + "%"
            : "";
          label = fmtTok(lens.tok) + " tok" + hitTxt;
          exactLabel = fmtInt(lens.tok) + " tok" + (lens.rate > 0 ? " · " + lens.rate.toFixed(1) + "%" : "");
        } else {
          label = "—";
        }
      }
      ...
      var body = function () {
        return renderTokenPanel({
          status: s1.status, convo: convo, agg2: agg2, recs: recs, modelRows: modelRows,
          convoTokens: convoTokens, turnTokens: turnTokens,
          stepStart: readWindow("step"), hasWindow: hasWindow, winTo: winTo
        });
      };
      ...
      return el("div", { style: { display: "inline-flex", alignItems: "center", gap: 4 } },
        el("button", { type: "button", key: "usage-tok",
          "data-dsh-usage-tokpill": "1",
          style: Object.assign({}, st.btn, { padding: "0 6px", minWidth: 0, fontSize: fs(11) }),
          title: exactLabel ? t("Token 明细") + " · " + exactLabel : t("Token 明细"),
    ...
    // 余额查询面板（DeepSeek / SiliconFlow / DigitalOcean / AMD GPU Cloud / 百炼 Token Plan）
    // 在本机不使用，因此关掉它的两个入口：对话页的「剩余余额查询」标签与设置页的同名分区。
    // 关掉的理由：该面板逐个服务商去请求官方余额接口，而本机 settings.yaml 里没有 DeepSeek
    // 供应商 —— 插件固定拿 DEEPSEEK_API_KEY 那条早已失效的凭据去问，一打开就固定 401，
    // 整个面板只剩报错。恢复方式：改回 true，两个入口一起回来（面板代码一直在）。
    var ENABLE_BALANCE_PANEL = false;
    ...
    function apply(ctx) {
      ...
      // 会话跳转动词。服务在点击那一刻才取，于是 apply 的时序不必依赖 sessions 已就绪；
      // 而「拿得到会话名」本身就说明列表服务在（useSessions 正是它提供的）。
      // sessions.open() 对不在 host 列表里的 id 会直接抛错 —— 这里吞掉，界面那侧本来
      // 也只给列表里存在的会话挂点击。
      function makeOpenSession(close) {
        return function (sid) {
          var svc = null;
          try { svc = ctx.get("sessions"); } catch (e) { svc = null; }
          if (!svc || typeof svc.open !== "function") return;
          try { svc.open(sid); } catch (e) { return; }
          // 从设置面板点进去时，先切会话再把面板收起，否则用户看不到跳转结果。
      ...
      slots.inject("conversation.view", function () {
        return slots.register(
          ...
          function (props) {
            return el("div", { style: st.tab }, el(UsagePanel, {
              timer: timer,
              useSessions: props && props.useSessions,
              openSession: makeOpenSession(null)
            }));
          }
        );
      });

      if (ENABLE_BALANCE_PANEL) {
        slots.inject("conversation.view", function () {
          return slots.register(
      ...
      slots.inject("settings.section", function () {
        return slots.register(
          ...
          function (props) {
            return el(UsagePanel, {
              timer: timer,
              useSessions: props && props.useSessions,
              openSession: makeOpenSession(props && props.close)
            });
          }
        );
      });
    ...
    // test/debug surface: session-row label and linkability decisions
    exports.__sessionRow = { view: sessionRowView, LABEL_MAX: SESSION_LABEL_MAX };
    // test/debug surface: the per-message turn window (snapshot walk + window shape)
    exports.__turnWindow = { field: turnWindowField, from: turnWindowFrom };
    // test/debug surface: the expanded panel body, renderable from a plain view object
    exports.__renderTokenPanel = renderTokenPanel;
    // test/debug surface: optional panels this build turns on
    exports.__slots = { balancePanelEnabled: ENABLE_BALANCE_PANEL };
