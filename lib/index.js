...
// 「计费输入」的唯一求和点——缓存命中率的分母，也是显示总量的前一半。
// 三个桶互斥：未命中输入、缓存命中、缓存写入。与官方 token-meter 的
// TokenUsageProjection 四个字段一一对应，命中率算法与 ui-chat 的
// StatsPills#billedInputTokens 保持一致，两个数字才对得上。
function billedInput(a) {
  return (a.cacheRead || 0) + (a.input || 0) + (a.cacheWrite || 0)
}

// 对已投影的记录做聚合（对话累计 / 本轮明细共用）。模块级纯函数而非闭包，
// 因为 test/token-accounting.test.js 要对它做回归——它是唯一的口径定义点，
// 口径错了界面上的数字就都错了，必须有测试钉住。
//
// 两个派生字段是给界面用的，别处不要重算：
//   billed       计费输入之和（命中率分母）
//   displayTotal 计费输入 + 输出，等价于官方 `message.turnUsage.count` 的口径
//
// 缓存命中率的分母曾漏掉 cacheWrite，于是命中率被系统性高估（本机全库
// 97.92% vs 实际 95.92%，差两个百分点），因为 cacheWrite 恰好在 DeepSeek 系
// provider 下等于 inputTokens，占分母三分之一量级。
// displayTotal 曾经把 reasoning 也加进来，而数据源已把它含在 outputTokens 内
// （本机记录里 reasoning 恒小于等于 output），等于重复计数。
export function computeAgg(items) {
  ...
  const denom = billedInput(agg)
  agg.hitRate = denom > 0 ? (agg.cacheRead / denom * 100) : 0
  agg.billed = denom
  agg.displayTotal = denom + agg.output
...
export default {
  ...
  apply(ctx) {
    ...
    try {
      ...
      // 聚合与计费口径定义在模块级（见 computeAgg / billedInput 的导出注释），
      // 这里不再复制一份：口径只能有一个定义点，否则测试锁住的和界面用的会是
      // 两份代码，迟早漂移。
      ...
      async function fetchOpenRouterPrices() {
        // OpenRouter 目录接近 1 MB，而 runCollect 给子进程 stdout 的上限是 64 KB
        // （maxBytes: 65536）。把响应体直接打到 stdout 会被静默截断，JSON.parse 随之
        // 报「Unexpected non-whitespace character after JSON」——看着像格式问题，
        // 实为长度上限。改为子进程写文件、只回传路径，宿主再读文件。
        const cachePath = joinPath(os.tmpdir(), 'dsh-usage-openrouter-catalog.json')
        const script = [
          'const https=require("https");',
          'const fs=require("fs");',
          'const out=' + JSON.stringify(cachePath) + ';',
          ...
          'res.on("end",()=>{',
          '  try{fs.writeFileSync(out,Buffer.concat(chunks));process.stdout.write(JSON.stringify({status:res.statusCode,path:out,bytes:Buffer.concat(chunks).length}));}',
        ...
        const filePath = typeof wrapper.path === 'string' && wrapper.path ? wrapper.path : cachePath
        let payload
        try {
          payload = JSON.parse(await nfsReadText(filePath))
        } finally {
